 ...
 * @LastEditTime: 2020-06-13 13:06:40
 */
    ...
    const float match_distance_maximum) 
{
  if (match_distance_maximum >= 0) {
    ...
    cout << "match distance maximum of HungarianMatcher is "
          << s_match_distance_maximum_<<endl;
    return true;
  }
  cout << "invalid match distance maximum of HungarianMatcher!"<<endl;
  return false;
}
    ...
    std::vector<int>* unassigned_tracks, std::vector<int>* unassigned_objects) 
{
  ...
  // cout << "HungarianMatcher: partition graph into " << track_components.size()
        //  << " sub-graphs."<<endl;
...
void HungarianMatcher::AssignObjectsToTracks(
  ...
  int no_object = association_mat.cols();
  // build cost
  std::vector<std::vector<float>> cost(no_track + no_object);
  for (int i = 0; i < no_track; ++i) {
