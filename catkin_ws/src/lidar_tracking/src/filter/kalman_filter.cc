 ...
 * @LastEditTime: 2020-06-10 16:17:22
 */
...
Eigen::Matrix3f KalmanFilter::s_propagation_noise_ = 10 * Eigen::Matrix3f::Identity();
...
void KalmanFilter::SetUseAdaptive(const bool& use_adaptive) 
{
  ...
  cout << "use adaptive of KalmanFilter is " << s_use_adaptive_<<endl;
}
    ...
    const double association_score_maximum) 
{
  if (association_score_maximum > 0) 
  {
    ...
    cout << "association score maximum of KalmanFilter is "
          << s_association_score_maximum_<<endl;
    return true;
  }
  cout << "invalid association score maximum of KalmanFilter!"<<endl;
  return false;
}
    ...
    const double breakdown_threshold_maximum) 
{
  if (breakdown_threshold_maximum > 0) 
  {
    ...
    cout << "breakdown threshold maximum of KalmanFilter is "
          << s_breakdown_threshold_maximum_<<endl;
    return true;
  }
  cout << "invalid breakdown threshold maximum of KalmanFilter!"<<endl;
  return false;
}
                              ...
                              const double z_propagation_noise) 
{
  ...
  {
    cout << "invalid measurement noise of KalmanFilter!"<<endl;
    return false;
  }
  if (initial_velocity_noise < 0) 
  {
    cout << "invalid intial velocity noise of KalmanFilter!"<<endl;
    return false;
  }
  if (xy_propagation_noise < 0) 
  {
    cout << "invalid xy propagation noise of KalmanFilter!"<<endl;
    return false;
  }
  if (z_propagation_noise < 0) 
  {
    cout << "invalid z propagation noise of KalmanFilter!"<<endl;
    return false;
  }
  ...
  cout << "measurment noise of KalmanFilter is " << s_measurement_noise_<<endl;
  cout << "initial velocity noise of KalmanFilter is "
        << s_initial_velocity_noise_<<endl;
  cout << "propagation noise of KalmanFilter is\n" << s_propagation_noise_<<endl;
  return true;
}
                              ...
                              const Eigen::Vector3f& velocity) 
{
  update_quality_ = 1.0;
  breakdown_threshold_ = s_breakdown_threshold_maximum_;
  belief_anchor_point_ = anchor_point.cast<float>();
    ...
    const std::shared_ptr<TrackedObject>& old_object, const double time_diff) 
{
  ...
  {
    cout << "Time diff is too limited to updating KalmanFilter!";
    return;
  }
    ...
    const std::shared_ptr<TrackedObject>& old_object, const double time_diff) 
{
  ...
  Eigen::Vector3f old_dir = old_object->direction.cast<float>();
  Eigen::Vector3f old_size = old_object->size.cast<float>();
  Eigen::Vector3f old_center = old_object->center.cast<float>();
  Eigen::Vector3f new_size = old_size;
  Eigen::Vector3f new_center = old_center;
  ComputeBboxSizeCenter<pcl::PointXYZI>(new_object->object_ptr->cloud, old_dir,
                                         &new_size, &new_center);
  ...
  Eigen::Vector3f project_dir =
      new_object->anchor_point - old_object->anchor_point;
  //如果求出来的中心速度方向和重心方向相反，这时候有干扰，中心速度暂时置为0。
...
{
  // Compute kalman gain
  Eigen::Matrix3f mat_c = Eigen::Matrix3f::Identity();
  Eigen::Matrix3f mat_q = s_measurement_noise_ * Eigen::Matrix3f::Identity();
  Eigen::Matrix3f mat_k =
      velocity_covariance_ * mat_c.transpose() *
      (mat_c * velocity_covariance_ * mat_c.transpose() + mat_q).inverse();

  // Compute posterior belief
  Eigen::Vector3f measured_anchor_point_d =
      measured_anchor_point.cast<float>();
  Eigen::Vector3f measured_velocity_d = measured_velocity.cast<float>();
  Eigen::Vector3f priori_velocity =
      belief_velocity_ + belief_acceleration_gain_ * time_diff;
  Eigen::Vector3f velocity_gain =
  ...
  //P_t
  velocity_covariance_ =
      (Eigen::Matrix3f::Identity() - mat_k * mat_c) * velocity_covariance_;
}
...
void KalmanFilter::EvaluateOnlineCovariance() 
{
  ...
  for (int i = 0; i < evaluate_window; ++i) 
  {
    ...
    {
      velocity_resisual =
          history_measured_velocity_[history_index].cast<float>() -
          belief_velocity_;
    }
    ...
    const Eigen::VectorXf& measured_acceleration) 
{
  // Compute kalman gain
  Eigen::Matrix3f mat_c = Eigen::Matrix3f::Identity();
  Eigen::Matrix3f mat_q =
      s_measurement_noise_ * Eigen::Matrix3f::Identity() * 3;
  Eigen::Matrix3f mat_k =
      velocity_covariance_ * mat_c.transpose() *
      (mat_c * velocity_covariance_ * mat_c.transpose() + mat_q).inverse();
  // Compute posterior belief
  Eigen::Vector3f measured_acceleration_d =
      measured_acceleration.cast<float>();
