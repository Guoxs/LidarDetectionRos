 ...
 * @LastEditTime: 2020-06-10 16:16:32
 */
    ...
    const int track_cached_history_size_maximum) 
{
  if (track_cached_history_size_maximum > 0) 
  {
    ...
    cout << "track cached history size maximum of object track is "
          << s_track_cached_history_size_maximum_<<endl;
    return true;
  }
  cout << "invalid track cached history size maximum of object track!"<<endl;
  return false;
}
...
bool ObjectTrack::SetSpeedNoiseMaximum(const double speed_noise_maximum) 
{
  if (speed_noise_maximum > 0) {
    ...
    cout << "speed noise maximum of object track is "
          << s_speed_noise_maximum_<<endl;
    return true;
  }
  cout << "invalid speed noise maximum of object track!"<<endl;
  return false;
}
    ...
    const double acceleration_noise_maximum) 
{
  if (acceleration_noise_maximum > 0) {
    ...
    cout << "acceleration noise maximum of object track is "
          << s_acceleration_noise_maximum_<<endl;
    return true;
  }
  cout << "invalid acceleration noise maximum of object track!"<<endl;
  return false;
}
...
ObjectTrack::ObjectTrack(std::shared_ptr<TrackedObject> obj) 
{
  ...
  // Initialize object direction with its lane direction,没有车道方向
                                      ...
                                      const double time_diff) 
{
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr pc = new_obj->object_ptr->cloud;
  ...
  // geometry_msgs::PolygonStampedPtr polygon(&(new_obj->object_ptr->polygon_));
  for (size_t j = 0; j < new_obj->object_ptr->polygon.points.size(); ++j) 
  {
    new_obj->object_ptr->polygon.points[j].x += predicted_shift[0];
    new_obj->object_ptr->polygon.points[j].y += predicted_shift[1];
    new_obj->object_ptr->polygon.points[j].z += predicted_shift[2];
  }
...
void ObjectTrack::SmoothTrackOrientation() 
{
  ...
  Eigen::Vector3f new_size;
  Eigen::Vector3f new_center;
  ComputeBboxSizeCenter<pcl::PointXYZI>(current_object_->object_ptr->cloud,
                                         current_dir.cast<float>(), &new_size,
                                         &new_center);
    ...
    const int track_consecutive_invisible_maximum) 
{
  if (track_consecutive_invisible_maximum >= 0) 
  {
    ...
    cout << "track consecutive invisible maximum of object track set is "
          << s_track_consecutive_invisible_maximum_<<endl;
    return true;
  }
  cout << "invalid track consecutive invisible maximum of object track! "<<endl;
  return false;
}
    ...
    const float track_visible_ratio_minimum) 
{
  if (track_visible_ratio_minimum >= 0 && track_visible_ratio_minimum <= 1) 
  {
    ...
    cout << "track visible ratio minimum of object track set is "
          << s_track_visible_ratio_minimum_<<endl;
    return true;
  }
  cout << "invalid track visible ratio minimum of object track! "<<endl;
  return false;
}
